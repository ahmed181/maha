	...
	*/
	public function TransferToProvider()
	{
		...
		$formFields = array(
			...
			'od_bill_address' 	=> $billingDetails['ordbillstreet1'] . ", " . $order['ordbillstreet2'],
			...
			'od_ship_city' 		=> $shippingAddress['suburb'],
		...
		foreach ($products as $product) {
			$productFields = array(
				'li_id'.$i			=> $product['product_code'],
				'li_description'.$i	=> $product['product_name'],
				'li_quantity'.$i 	=> $product['quantity'],
				'li_price'.$i 		=> number_format($product['product_price'], 2,'.','')
			);
